 ...
 */
@Slf4j
public class MyExcelWriteFillExecutor extends ExcelWriteFillExecutor {
     ...
     */
    private final Map<ExcelWriteFillExecutor.UniqueDataFlagKey, List<AnalysisCell>> templateAnalysisCache = MapUtils.newHashMap();
    /**
     * Collection fields to replace in the template
     */
    private final Map<ExcelWriteFillExecutor.UniqueDataFlagKey, List<AnalysisCell>> templateCollectionAnalysisCache = MapUtils.newHashMap();
    /**
     * Style cache for collection fields
     */
    private final Map<ExcelWriteFillExecutor.UniqueDataFlagKey, Map<AnalysisCell, CellStyle>> collectionFieldStyleCache
            = MapUtils.newHashMap();
    /**
     * Row height cache for collection
     */
    private final Map<ExcelWriteFillExecutor.UniqueDataFlagKey, Short> collectionRowHeightCache = MapUtils.newHashMap();
    /**
     * Last index cache for collection fields
     */
    private final Map<ExcelWriteFillExecutor.UniqueDataFlagKey, Map<AnalysisCell, Integer>> collectionLastIndexCache = MapUtils.newHashMap();

    private final Map<ExcelWriteFillExecutor.UniqueDataFlagKey, Integer> relativeRowIndexMap = MapUtils.newHashMap();
    /**
     * The unique data encoding for this fill
     */
    private ExcelWriteFillExecutor.UniqueDataFlagKey currentUniqueDataFlag;

    public MyExcelWriteFillExecutor(WriteContext writeContext) {
        super(writeContext);
    }
    ...
    private void increaseRowIndex(Map<ExcelWriteFillExecutor.UniqueDataFlagKey, List<AnalysisCell>> templateAnalysisCache, int number,
                                  int maxRowIndex) {
        for (Map.Entry<ExcelWriteFillExecutor.UniqueDataFlagKey, List<AnalysisCell>> entry : templateAnalysisCache.entrySet()) {
            ExcelWriteFillExecutor.UniqueDataFlagKey uniqueDataFlagKey = entry.getKey();
    ...
    private String prepareData(Cell cell, int rowIndex, int columnIndex,
                               Map<ExcelWriteFillExecutor.UniqueDataFlagKey, Set<Integer>> firstRowCache) {
    ...
    private String dealAnalysisCell(AnalysisCell analysisCell, String value, int rowIndex, int lastPrepareDataIndex,
                                    int length, Map<ExcelWriteFillExecutor.UniqueDataFlagKey, Set<Integer>> firstRowCache, StringBuilder preparedData) {
        if (analysisCell != null) {
            ...
            ExcelWriteFillExecutor.UniqueDataFlagKey uniqueDataFlag = uniqueDataFlag(writeContext.writeSheetHolder(),
    ...
    private ExcelWriteFillExecutor.UniqueDataFlagKey uniqueDataFlag(WriteSheetHolder writeSheetHolder, String wrapperName) {
        return new ExcelWriteFillExecutor.UniqueDataFlagKey(writeSheetHolder.getSheetNo(), writeSheetHolder.getSheetName(), wrapperName);
    }
}
